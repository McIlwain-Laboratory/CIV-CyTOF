...
for(tissue_type in tissue_vector){
  ...
  subset_df <- combined_Marker_data %>% filter(tissue == tissue_type & stim == stim_type) #%>% 
    #group_by(metadata_test) %>% 
    #    slice_min(adjusted_p_value, n = 3) %>% ungroup()
...
ggsave(filename = "output/correlation_plots/marker_corr_heatmap_Jan21.pdf", plot = combined_plots, width = 10, height = 6, dpi = 300)
